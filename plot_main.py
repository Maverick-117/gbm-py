...
for lll_load in lll_vec:
    ...
    plot_drty = "C:\\Users\\jhvo9\\Google Drive (vojh1@uci.edu)\\a PhD Projects\\GBM Modeling\\python scripts\\plots\\kim_model"+model_suffix+"\\"+date_dir+deathVal_dir + sub_drty + fdbk_dirs + log_dir
    ...
    if log_yscaleQ:
        axD.set_yscale('log')
    axD.plot(t_vec[0][:st2_stop],hd*u_dc[0][:st2_stop]**n/(1+hd*u_dc[0][:st2_stop]**n),color[0]+'-',label='prob feedback '+str(csr[0])+unit)
    axD.plot(t_vec[1][:st1_stop],hd*u_dc[1][:st1_stop]**n/(1+hd*u_dc[1][:st1_stop]**n),color[1]+'-',label='prob feedback '+str(csr[1])+unit)
    axD.plot(tn_vec[0][:st0_stop],hd*un_dc[1][:st0_stop]**n/(1+hd*un_dc[1][:st0_stop]**n),color[2]+'-',label='prob feedback, no treatment')
    axD.legend();
